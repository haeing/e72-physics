...
static const double trigger_eff = 0.5;
...
static const double p_start = 575;
static const double p_end = 1003;
const int np = 428/2;
...
void yield(){
  ...
  */
  ...
  TFile *file_nbeam = new TFile("data/beam/Nbeam_251123.root","READ");
  TH1D *hist_beamall = (TH1D*)file_nbeam->Get("hMom2");
  TCanvas *c10 = new TCanvas("c10","c10");
  hist_beamall->Draw();
  ...
  for(int i=0;i<np;i++){
    ...
    yield_cal[i] = Nbeam_point*Ntarget*cs*mb_to_cm2*E_eff*trigger_eff*fraction_L;
    // Preserve the measured beam histogram; yield is stored separately.
  ...
  yield_total->SetTitle(";#it{p_{K}} [MeV/#it{c}];Yield / [2 MeV/c]");
  yield_total->Draw("AP");
  //show sqrt s
  ...
  cout<<"N_beam (measured histogram integral) : "<<hist_beamall->Integral()<<endl;
  cout<<"N_target : "<<Ntarget<<endl;
  cout<<"Total yield : "<<yield_total->Integral()<<endl;
  

  gSystem->mkdir("results", kTRUE);
  TFile output("results/yield.root", "RECREATE");
  TGraphErrors entry_graph(n_mombin), lumi_graph(n_mombin);
  for(int i=0;i<n_mombin;i++){
    ...
    entry_graph.SetPoint(i,mom_kaon[i],entry_mom);
    entry_graph.SetPointError(i,mom_kaon_err,TMath::Sqrt(entry_mom));
    lumi_graph.SetPoint(i,mom_kaon[i],lumi_mom);
    lumi_graph.SetPointError(i,mom_kaon_err,0.);
    RootData::momentum(output,mom_kaon[i]);
    TParameter<double>("yield",entry_mom).Write();
    TParameter<double>("luminosity",lumi_mom).Write();
  ...
  output.cd();
  entry_graph.Write("entry_mom");
  lumi_graph.Write("luminosity_mom");
  yield_total->Write("yield_spectrum");
  lumi_total->Write("luminosity_spectrum");
  hist_beamall->Write("beam_momentum");
  Leta1->Write("cross_section_reference_1");
  Leta2->Write("cross_section_reference_2");
  Leta_total->Write("cross_section_reference_combined");
  cs_val->Write("cross_section_interpolated");
  output.Close();
}
