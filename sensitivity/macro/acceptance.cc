...
void acceptance(){
  ...
  //TFile *file = new TFile("data/mc/mom735_flat_eta_lambda.root");
  ...
  TFile output("results/acceptance.root", "RECREATE");
  for (int i=0;i<n_mombin;++i) {
    RootData::momentum(output,mom_kaon[i]);
    RootData::angular(Leta_accept[i],"acceptance");
    ...
    passed.Write();
    TH1D generated("mc_total","Generated MC;cos theta;Events",n_cosbin,-1,1);
    generated.SetDirectory(nullptr);
    for (int j=0;j<n_cosbin;++j) {
      generated.SetBinContent(j+1,total[i][j]);
      generated.SetBinError(j+1,std::sqrt(total[i][j]));
    }
    generated.Write();
  }
  output.cd();
  RootData::angular(Leta_accept_all,"acceptance_all");
  output.Close();
}
