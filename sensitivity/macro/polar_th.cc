...
void polar_th() {
  ...
  RootData::readAngular("results/diff_cross.root","yield",mom_kaon,N_mom_cos);
  for (int i=0;i<n_mombin;++i)
    for (int j=0;j<n_cosbin;++j) N_mom[i]+=N_mom_cos[i][j];

  double N_mom_cos_compare[n_cosbin] = {0.};
  for(int i=0;i<10;i++){
    for(int j=0;j<n_cosbin;j++){
      N_mom_cos_compare[j] +=N_mom_cos[i][j];
    }
  }
  ...
  for (int mom = 0; mom < 2; mom++) {
	...
	if(mom==0){
	  ...
	  model_value_err[mom][theory][i] = sqrt(3)/(alpha*sqrt((double)N_mom_cos_compare[i]));
	}
	else if(mom==1){
  ...
  TFile output("results/polar_th.root","RECREATE");
  gr_polar_total->Write("polarization_uncertainty_momentum");
  for (int i=0;i<n_mombin;++i) {
    RootData::momentum(output,mom_kaon[i]);
    TGraphErrors uncertainty(n_cosbin);
    uncertainty.SetTitle(";cos theta;Statistical uncertainty of P_Lambda");
    for (int j=0;j<n_cosbin;++j) {
      uncertainty.SetPoint(j,cos_cm[j],sqrt(3)/(alpha*sqrt(N_mom_cos[i][j])));
      uncertainty.SetPointError(j,cos_cm_err[j],0.);
    }
    RootData::angular(&uncertainty,"polarization_uncertainty");
  }
  output.cd();
  auto *comparison=output.mkdir("model_comparison");
  for (int m=0;m<2;++m) {
    comparison->cd();
    auto *dir=comparison->mkdir(m==0 ? "model735_sum724_750" : "model765_bin766");
    dir->cd();
    gr_exp[m]->Write("crystal_ball");
    for (int t=0;t<2;++t) {
      gr_model[m][t]->Write(t==0 ? "D03" : "P03");
      spline[m][t]->Write(t==0 ? "spline_D03" : "spline_P03");
    }
  }
  output.Close();
}
